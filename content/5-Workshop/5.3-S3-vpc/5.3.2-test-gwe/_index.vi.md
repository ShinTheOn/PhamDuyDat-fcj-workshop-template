...
+ Trong workshop này, bạn sẽ dùng **AWS Session Manager** để kết nối đến các EC2 instances. **Session Manager** là một tính năng trong dịch vụ **AWS Systems Manager** được quản lý hoàn toàn. **Systems Manager** cho phép bạn quản lý các Amazon EC2 instances và các máy ảo on-premises (VMs) thông qua một browser-based shell. Session Manager cung cấp khả năng quản lý phiên bản an toàn và dễ kiểm tra mà không cần mở cổng vào, duy trì bastion host hoặc quản lý khóa SSH.

+ Xem lab First cloud journey để hiểu sâu hơn về Session Manager.
...
1. Đổi về thư mục của `ssm-user` bằng lệnh `cd ~`.
