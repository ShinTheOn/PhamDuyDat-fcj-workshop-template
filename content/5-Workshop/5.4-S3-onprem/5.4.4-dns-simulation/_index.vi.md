...
+ `Choose endpoint`: dán tên Regional VPC Endpoint DNS bạn đã lưu ở phần 4.3.

![record1](/images/5-Workshop/5.4-S3-onprem/record1.png)

3. Nhấp vào **Add another record**, và thêm một bản ghi thứ hai với các thông số sau:
+ `Record name`: `*.`
+ `Record type`: giữ giá trị mặc định (type A).
+ Bật `Alias`.
+ `Route traffic to`: Alias to VPC Endpoint.
+ `Region`: US East (N. Virginia) [us-east-1].
+ `Choose endpoint`: dán tên Regional VPC Endpoint DNS bạn đã lưu ở phần 4.3.
...
+ Target IP Addresses: điền cả hai IP bạn đã lưu trữ trên trình soạn thảo (inbound endpoint addresses), sau đó chọn **Submit**.
...
Trong phần này, bạn đã tạo một **Interface Endpoint** cho Amazon S3. Điểm cuối này có thể được truy cập từ on-premises thông qua Site-to-Site VPN hoặc AWS Direct Connect. Các điểm cuối Route 53 Resolver outbound mô phỏng chuyển tiếp các yêu cầu DNS từ on-premises đến một Private Hosted Zone đang chạy trên đám mây. Các điểm cuối Route 53 inbound nhận yêu cầu phân giải và trả về phản hồi chứa địa chỉ IP của **Interface Endpoint** VPC. Sử dụng DNS để phân giải địa chỉ IP của endpoint giúp tăng tính sẵn sàng khi một Availability Zone gặp sự cố.
